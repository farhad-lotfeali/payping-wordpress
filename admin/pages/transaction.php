...
function pp_transaction_list()
{
    ...
    if(isset($_GET['from']) && !empty($_GET['from'])){
        ...
        $filters['fromDate'] = jalali_to_gregorian(intval(convert($from[0])),intval(convert($from[1])),intval(convert($from[2])),'-');
    }

    if(isset($_GET['to']) && !empty($_GET['to'])){
        $to = explode('/',$_GET['to']);
        $filters['toDate'] = jalali_to_gregorian(intval(convert($to[0])),intval(convert($to[1])),intval(convert($to[2])),'-');
    }
